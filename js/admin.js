...
(function () {
  ...
  function assertAdmin() {
    ...
    // still allow your temporary session fallback if Identity isn’t ready:
  ...
  // Standard logout (Identity + session)
  ...
  // Keeps your localStorage data model and all UI behavior you already use.
  ...
  const KEYS = {
    accounts:'tnk_accounts', timesheets:'tnk_timesheets', prices:'tnk_prices',
    promos:'tnk_promotions', invoices:'tnk_invoices', services:'tnk_services',
    jobs:'tnk_jobs', reviews:'tnk_reviews', pto:'tnk_pto', paystubs:'tnk_paystubs',
    availability:'tnk_availability'
  };

  // ---- seed (same as you had) ----
  (function seed(){
  ...
  // ---- ACCOUNTS (same behavior) ----
  const accForm = byId('account-form');
  const accTableBody = $('#acc_table tbody');
  const accFilterRole = byId('acc_filter_role');
  const accSearch = byId('acc_search');
  const accFields = {
    id: byId('acc_id'), role: byId('acc_role'), email: byId('acc_email'),
    name: byId('acc_name'), phone: byId('acc_phone'), address: byId('acc_address')
  };
  ...
  function renderAccounts(){
    ...
    accTableBody.innerHTML = loadAccounts()
      ...
      .map(a=>`
        ...
        </tr>`).join('') || '<tr><td colspan="6" class="muted">No accounts.</td></tr>';
  }
  accForm?.addEventListener('submit', (e)=>{
    e.preventDefault();
    const id = accFields.id.value || crypto.randomUUID();
    const data = {
      id, role: accFields.role.value,
    ...
    saveAccounts(list); accStatus.textContent='Saved.'; accForm.reset(); accFields.id.value='';
  });
  ...
  accTableBody?.addEventListener('click', (e)=>{
    ...
    const id = tr.dataset.id; const list = loadAccounts(); const a = list.find(x=>x.id===id);
  ...
  // ---- Select refreshers used elsewhere ----
  function refreshSelects(){
    const customers = loadAccounts().filter(a=>a.role==='customer');
    const employees = loadAccounts().filter(a=>a.role==='employee');
    const dlCust = byId('customer-emails'); if(dlCust) dlCust.innerHTML = customers.map(c=>`<option value="${c.email}">`).join('');
    ...
    const dlEmp = byId('employee-emails'); if(dlEmp) dlEmp.innerHTML = employees.map(e=>`<option value="${e.email}">`).join('');
  ...
  // ---- SCHEDULE/JOBS (same behavior) ----
  const jobForm = byId('job-form'); const jobStatus = byId('job_status_msg');
  ...
  function renderAdminCalendar(){
    ...
    const jobs = loadJobs().sort((a,b)=> (a.date||'').localeCompare(b.date||'') || (a.start||'').localeCompare(b.start||''));
    ...
    cal.onclick = (e)=>{
      ...
      const id=el.dataset.id; const j=loadJobs().find(x=>x.id===id); if(!j) return;
      // Fill form
      ['job_id','job_customer','job_title','job_date','job_start','job_end','job_assignee','job_invoice','job_lat','job_lon','job_status','job_notes']
        .forEach(k=>{ const el=byId(k); if(el) el.value = j[k.replace('job_','')] || ''; });
  ...
  jobForm?.addEventListener('submit',(e)=>{
    ...
    const list = loadJobs(); const i = list.findIndex(x=>x.id===id);
    if(i>=0) list[i]=data; else list.push(data);
    saveJobs(list); jobStatus.textContent='Saved.'; jobForm.reset(); byId('job_id').value='';
  });
  byId('job_reset')?.addEventListener('click', ()=>{ jobForm.reset(); byId('job_id').value=''; jobStatus.textContent=''; });

  // ---- INVOICES (same behavior) ----
  ...
  function autoInvoiceNumber(){ const k='tnk_inv_counter'; let n=Number(localStorage.getItem(k)||1000); n+=1; localStorage.setItem(k,String(n)); return `INV-${n}`; }
  function addItemRow(item={desc:'',qty:1,unit:35}) {
    ...
    row.addEventListener('input', recalcInvoice); row.querySelector('.js-del-item').addEventListener('click',()=>{row.remove();recalcInvoice();});
    recalcInvoice();
  }
  function invoiceItems(){ return $$('.items-row', invItemsWrap).map(r=>({desc:$('.it-desc',r).value.trim(), qty:Number($('.it-qty',r).value||0), unit:Number($('.it-unit',r).value||0)})).filter(i=>i.desc||i.qty||i.unit); }
  function recalcInvoice(){
    const items=invoiceItems(); const sub=items.reduce((s,i)=>s+i.qty*i.unit,0);
    const taxPct=Number(byId('inv_tax').value||0); const total=sub+sub*(taxPct/100);
    invTotalOut.textContent = money(total);
  }
  invAddItemBtn?.addEventListener('click',()=>addItemRow());
  function resetInvoiceForm(){
    byId('inv_id').value=''; byId('inv_customer').value=''; byId('inv_number').value=autoInvoiceNumber();
    byId('inv_status').value='unpaid'; byId('inv_date').valueAsDate=new Date(); byId('inv_due').value='';
    byId('inv_tax').value='0'; byId('inv_notes').value=''; invItemsWrap.innerHTML=''; addItemRow({desc:'Mowing / Trimming', qty:1, unit:35}); invStatusMsg.textContent=''; recalcInvoice();
  }
  invForm?.addEventListener('submit',(e)=>{
    ...
    const taxPct=Number(byId('inv_tax').value||0); const total=subtotal+subtotal*(taxPct/100);
    const data={ id, customer_email: byId('inv_customer').value.trim(), number: byId('inv_number').value.trim()||autoInvoiceNumber(),
      status: byId('inv_status').value, date: byId('inv_date').value, due: byId('inv_due').value, tax_pct: taxPct, items,
      subtotal, tax: subtotal*(taxPct/100), total, notes: byId('inv_notes').value.trim() };
    const list=loadInvoices(); const i=list.findIndex(x=>x.id===id); if(i>=0) list[i]=data; else list.push(data);
    saveInvoices(list); invStatusMsg.textContent='Invoice saved.'; resetInvoiceForm();
  });
  invResetBtn?.addEventListener('click', resetInvoiceForm);
  byId('inv_tax')?.addEventListener('input', recalcInvoice);
  function renderInvoices(){
    const list=loadInvoices();
    const fCust=fInvCustomer?.value||'all'; const fStat=fInvStatus?.value||'all'; const q=(fInvQ?.value||'').toLowerCase();
    invTableBody.innerHTML = list
      ...
      .map(i=>`
        ...
        </tr>`).join('') || '<tr><td colspan="7" class="muted">No invoices.</td></tr>';
  }
  invTableBody?.addEventListener('click',(e)=>{
    const tr=e.target.closest('tr'); if(!tr) return; const id=tr.dataset.id; const list=loadInvoices(); const i=list.find(x=>x.id===id);
    if(e.target.classList.contains('js-edit')) {
    ...
    if(e.target.classList.contains('js-toggle')) { i.status = i.status==='paid'?'unpaid':'paid'; saveInvoices(list); }
    if(e.target.classList.contains('js-del')) { if(confirm('Delete this invoice?')) saveInvoices(list.filter(x=>x.id!==id)); }
  ...
  byId('inv_export_csv')?.addEventListener('click', ()=>{
    const list=loadInvoices();
    const rows=[['number','customer_email','date','due','status','subtotal','tax','total','notes'],
  ...
  // ---- TIMESHEETS (same) ----
  const tsForm = byId('ts-form'); const tsStatus = byId('ts_status'); const tsTableBody = $('#ts_table tbody');
  const tsFilterEmail = byId('ts_filter_email'); const tsFilterFrom = byId('ts_filter_from'); const tsFilterTo = byId('ts_filter_to');
  ...
  function hours(a,b){ return Math.max(0, parseH(a)-0 + (parseH(b))); } // simple
  ...
  tsForm?.addEventListener('submit',(e)=>{
    ...
    const data={ id, employee_email: byId('ts_employee_email').value.trim(), date: byId('ts_date').value,
      start_time: byId('ts_start').value, end_time: byId('ts_end').value, notes: byId('ts_notes').value.trim(), approved:false };
    const list=loadTimesheets(); const i=list.findIndex(x=>x.id===id); if(i>=0){ data.approved=!!list[i].approved; list[i]=data; } else list.push(data);
    saveTimesheets(list); tsStatus.textContent='Saved.'; tsForm.reset(); byId('ts_id').value='';
  });
  ...
  tsTableBody?.addEventListener('click',(e)=>{
    const tr=e.target.closest('tr'); if(!tr) return; const id=tr.dataset.id; const list=loadTimesheets(); const t=list.find(x=>x.id===id);
    ...
    if(e.target.classList.contains('btn-del')){ if(confirm('Delete this timesheet?')) saveTimesheets(list.filter(x=>x.id!==id)); }
    if(e.target.classList.contains('btn-approve')){ t.approved=!t.approved; saveTimesheets(list); }
  });
  function renderTimesheets(){
    ...
    tsTableBody.innerHTML = list
      ...
      .map(t=>`<tr data-id="${t.id}">
        ...
        <td><button class="btn-small btn-edit">Edit</button><button class="btn-small btn-small--danger btn-del">Delete</button></td></tr>`).join('') || '<tr><td colspan="8" class="muted">No entries.</td></tr>';
  }

  // ---- SERVICES/REVIEWS/PTO/PROMOS/PRICES initial renders ----
  function renderServices(){ const tbody=$('#svc_table tbody'); if(!tbody) return; const list=store.get(KEYS.services,[]); tbody.innerHTML = list.map(s=>`<tr data-id="${s.id}"><td>${s.tier}</td><td>${s.name}</td><td>${money(s.price_from)}</td><td>${s.desc||''}</td><td></td></tr>`).join('') || '<tr><td colspan="5" class="muted">No services yet.</td></tr>'; }
  function renderReviews(){ const tbody=$('#review_table tbody'); if(!tbody) return; const list=store.get(KEYS.reviews,[]); tbody.innerHTML = list.map(r=>`<tr><td>${r.date||''}</td><td>${r.customer||''}</td><td>${r.title||''}</td><td>${(r.photos||[]).length}</td><td>${r.notes||''}</td><td>${r.status||'pending'}</td></tr>`).join('') || '<tr><td colspan="6" class="muted">No completed jobs pending.</td></tr>'; }
  function renderPTO(){ const tbody=$('#pto_table tbody'); if(!tbody) return; const list=store.get(KEYS.pto,[]); tbody.innerHTML = list.map(p=>`<tr><td>${p.employee||''}</td><td>${p.from||''}</td><td>${p.to||''}</td><td>${p.reason||''}</td><td>${p.status||'pending'}</td><td></td></tr>`).join('') || '<tr><td colspan="6" class="muted">No requests.</td></tr>'; }

  // promos
  const promoForm = byId('promo-form'); const promoStatus = byId('promo_status');
  promoForm?.addEventListener('submit',(e)=>{ e.preventDefault(); store.set(KEYS.promos, { title: byId('promo_title').value.trim(), subtitle: byId('promo_subtitle').value.trim(), active: byId('promo_active').value==='true' }); promoStatus.textContent='Promotion saved.'; });
  const priceForm = byId('price-form'); const priceStatus = byId('price_status');
  priceForm?.addEventListener('submit',(e)=>{ e.preventDefault(); store.set(KEYS.prices, { essential:Number(byId('p_essential').value||35), standard:Number(byId('p_standard').value||55), premium:Number(byId('p_premium').value||85) }); priceStatus.textContent='Prices saved.'; });
